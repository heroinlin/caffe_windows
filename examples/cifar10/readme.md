点击create_cifar10.bat生成cifar10_train_leveldb和cifar10_testleveldb文件夹,点击run_compute_image_mean.bat生成./mean.binaryproto文件，cifar10_train_leveldb文件夹中也有相应的更新，由于生成数据类型为leveldb类型，所以需要更改cifar10_quick_train_test.prototxt文件中的lmdb(大小写)为leveldb,最后点击run_cifar10.bat运行即可。
create_cifar10.bat(在example/cifar10目录下):
                 convert_cifar10_data.exe  ../../data/cifar10 ../cifar10 leveldb
				 pause
run_compute_image_mean.bat(在caffeRoot目录下，即archive目录):
                 .\bin\compute_image_mean --backend=leveldb examples/cifar10/cifar10_train_leveldb examples/cifar10/mean.binaryproto
				 pause
run_cifar10.bat (在caffeRoot目录下，即archive目录):
